...
/**
 * Para clínicas — alta lead form. No DB endpoint exists yet, so the
 * form opens a prefilled mailto: to operations. When /api/clinics/leads
 * is built we swap the handler over.
 */
export default function ClinicLeadForm() {
  const [submitted, setSubmitted] = useState(false);
  const [form, setForm] = useState({
    clinicName: '', contactName: '', email: '', phone: '', insurers: '',
  });
  ...
  function handleSubmit(e) {
    ...
    const body = encodeURIComponent(
      `Solicitud de alta — clínica\n\n` +
      `Clínica: ${form.clinicName}\nContacto: ${form.contactName}\n` +
      `Email: ${form.email}\nTeléfono: ${form.phone}\nAseguradoras: ${form.insurers}\n`
    );
    window.location.href = `mailto:hola@medconnect.es?subject=Alta%20cl%C3%ADnica&body=${body}`;
    setSubmitted(true);
  }

  return (
    <form className="form-card" onSubmit={handleSubmit}>
      <Input label="Nombre de la clínica" placeholder="Centro Médico Velázquez" value={form.clinicName} onChange={(e) => handleChange('clinicName', e.target.value)} required />
      ...
      <Input label="Aseguradoras concertadas" placeholder="Sanitas, Adeslas, DKV…" value={form.insurers} onChange={(e) => handleChange('insurers', e.target.value)} required />
