...
export default function ReferralModal({
  ...
  // can list 50+ procedures otherwise). Default carries the step-1
  // specialty over; the user can clear it on step 2.
  ...
  // Defensive slug normalization — the `clinic_procedures` table is
  // populated from multiple sources (manual entry, SON catalogue import)
  // and a few rows arrive with stray whitespace or mixed case on the
  // `specialty_slug` column. Without normalization the equality check
  // below silently no-ops on those rows and the user sees the dropdown
  // appear but the procedure list never narrow — which is exactly the
  // regression that prompted this fix.
  const normalizeSlug = (s) => (typeof s === 'string' ? s.trim().toLowerCase() : '');

  // Distinct specialties present in the selected clinic's catalogue —
  // drives the step-2 dropdown. Only render the filter when more than
  // one specialty is present (single-specialty clinics don't need it).
  const step2Specialties = useMemo(() => {
    const map = new Map();
    for (const p of procedures) {
      const slug = normalizeSlug(p.specialtySlug);
      if (slug && !map.has(slug)) {
        map.set(slug, { slug, name: p.specialtyName || p.specialtySlug });
      }
    }
    return [...map.values()].sort((a, b) => a.name.localeCompare(b.name, 'es'));
  }, [procedures]);

  // Filter the procedure list by the selected specialty. Comparison is
  // normalized on both sides so case / whitespace drift in the DB
  // doesn't break the filter.
  const step2FilteredProcedures = useMemo(() => {
    const target = normalizeSlug(step2SpecialtySlug);
    if (!target) return procedures;
    return procedures.filter((p) => normalizeSlug(p.specialtySlug) === target);
              ...
              {proceduresLoading ? (
                ...
                <>
                  {step2Specialties.length > 1 && (
                    <div className="referral-filters">
                      <select
                        ...
                        aria-label="Filtrar actos médicos por especialidad"
                      >
                        <option value="">Todas las especialidades</option>
                  ...
                  {step2FilteredProcedures.length === 0 ? (
                    <div className="no-slots">No hay actos médicos de esta especialidad en esta clínica.</div>
                  ) : (
