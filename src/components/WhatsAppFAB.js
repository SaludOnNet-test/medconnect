 ...
 *
 * 2026-06-04 v2 — added an intercept modal (greeting + business hours)
 * because back then a human answered and expectations needed setting.
 * 2026-07-16 v3 — modal REMOVED: the AI assistant answers 24/7 within
 * seconds, so the expectation-setting step is pure friction now. The FAB
 * deep-links straight to wa.me. Both funnel events still fire on the
 * single click so existing dashboards (fab_opened vs continue_clicked)
...
export default function WhatsAppFAB() {
  ...
  const handleFabClick = () => {
    try {
      ...
      trackEvent('whatsapp_continue_clicked', { source: 'fab', path: pathname, direct: true });
